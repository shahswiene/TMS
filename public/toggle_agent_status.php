...
if (empty($agent_id) || empty($new_status)) {
    echo json_encode(['success' => false, 'message' => 'Agent ID and new status are required']);
    exit;
}

try {
    $stmt = $pdo->prepare('UPDATE wazuh_agent SET status = ? WHERE agent_id = ?');
    $stmt->execute([$new_status, $agent_id]);
    
    echo json_encode(['success' => true, 'message' => 'Agent status updated successfully']);
} catch (PDOException $e) {
    error_log($e->getMessage());
    echo json_encode(['success' => false, 'message' => 'An error occurred while updating agent status']);
}
?>